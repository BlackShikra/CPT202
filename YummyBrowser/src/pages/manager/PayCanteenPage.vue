<template>
  <div>
          ...
          <div slot="content">
            <table>
              <tr>
                ...
                <td rowspan="4">
                  <Button type="warning" @click="payOrder(index1)">转 账</Button>
                </td>
              </tr>
              <tr>
                <th>优惠合计：</th>
    ...
    <div v-if="showNotPay&&notPayed.length==0"
         style="float: left;background-color: rgb(247,247,247);height: 100px;margin-left: 15%;width: 70%;font-size: 15px;font-weight: bold;padding-top: 40px">
      暂无未支付订单
    </div>
          ...
          <div slot="content">
            <table>
              <tr>
                <th>菜品合计：</th>
                <td>{{ getPrice(order.dishSum) }}</td>
                <td rowspan="5"><p style="-webkit-text-fill-color: #19be6b;font-weight: bold;font-size: 13px;">已支付</p>
                </td>
              </tr>
              <tr>
                <th>优惠合计：</th>
    ...
    <div v-if="showPay&&payed.length==0"
         style="float: left;background-color: rgb(247,247,247);height: 100px;margin-left: 15%;width: 70%;font-size: 15px;font-weight: bold;padding-top: 40px">
      暂无已支付订单
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    async initial() {
      await this.$axios.post('/server/manage/get/unpaid', {}).then(re => {
        this.notPayed = re.data;
        console.log(this.notPayed)
      })
      await this.$axios.post('/server/manage/get/paid', {}).then(re => {
        this.payed = re.data;
